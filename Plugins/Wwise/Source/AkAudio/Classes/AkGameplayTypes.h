...
struct FAkExternalSourceInfo
{
	...
	FString ExternalSrcName;	///< Name of the source given in the project. \aknote If an event triggers the playback of more than one external source, they must be named uniquely in the project therefore have a unique cookie) in order to tell them apart when filling the AkExternalSourceInfo structures. \endaknote
	
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Audiokinetic|AkExternalSourceInfo")
	AkCodecId CodecID;			///< Codec ID for the file.  One of the audio formats defined in AkTypes.h (AKCODECID_XXX)
	
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Audiokinetic|AkExternalSourceInfo")
	FString FileName;			///< File path for the source.  If not NULL, the source will be streaming from disk.  Set pInMemory to NULL. If idFile is set, this field is used as stream name (for profiling purposes).
};

struct FAkSDKExtrernalSourceArray
{
	FAkSDKExtrernalSourceArray(const TArray<FAkExternalSourceInfo>& BlueprintArray);
	~FAkSDKExtrernalSourceArray();
